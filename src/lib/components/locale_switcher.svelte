<script lang="ts">
  ...
  // update locale when select element changes
  const handleChange = (event: Event) => {
    const selectElement = event.target as HTMLSelectElement;

    if (selectElement) {
      window.location.assign(selectElement.value);
    }
  };
</script>

<select on:change={handleChange} style="background-color: var(--qwer-bg-color);">
  {#each locales as l}
    {#if l === $locale}
